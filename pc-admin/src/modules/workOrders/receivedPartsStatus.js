...
const getOrderStatus = (order = {}) => (order ? String(order.statusEn || order.status || '').trim() : '')

const getReceiptStatus = (order = {}) => (order ? String(
...
export const selectPreferredDrawerStatus = ({ order = {}, allowedStatuses = [], activeTab = '' } = {}) => {
  if (!Array.isArray(allowedStatuses) || !allowedStatuses.length) return (order && order.status) || ''
