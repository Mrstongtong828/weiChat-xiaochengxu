...
- 财务先在工单抽屉“收款”页核对银行对公流水并点击“审核通过”；状态允许时，工单会自动进入处理中。
- 微信支付订单不能手工确认到账，也不显示发票登记入口。只有已完成且已核销的对公转账订单可以登记发票。
- 工单抽屉“发票”页展示发票状态、抬头、税号和备注，开票模式固定为“人工开票并登记”。
- 税收分类固定为“修理修配劳务”，发票项目固定为“牙科设备检修服务费”；一个发票号码只能绑定一个工单。
- 后台更新发票状态时写回 `invoice_info`，并保留客户提交的申请字段。已开具的电子票原件上传到项目云存储后，对应订单客户可在小程序查看和下载 PDF；不得使用不受控的外部 PDF 地址。
- 电子普通发票按 1-3 个工作日处理；纸质专用发票按 7-15 个工作日寄出，并在后台登记快递公司和单号。
- 验收时需覆盖：微信支付订单无入口、未完成或未核销对公单被拒绝、重复发票号被拒绝、已开发票在小程序结构化展示和 PDF 查看/下载，以及跨账号不可访问。
