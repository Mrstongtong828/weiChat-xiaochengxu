...
assert.match(viewSource, /wo\.filter === '处理中'[\s\S]*?\['inspecting', 'fixing'\]/)
assert.match(viewSource, /const clearStatusCardConflicts = \(\) => \{[\s\S]*?wo\.search = ''[\s\S]*?searchInvoiceStatus\.value = ''[\s\S]*?wo\.warrantyFilter = ''[\s\S]*?wo\.customerTypeFilter = ''[\s\S]*?slaFilter\.value = ''[\s\S]*?activeTodoType\.value = ''/)
assert.match(backendSource, /next === 'sent' && currentStatus === 'pending'/)
assert.match(backendSource, /if \(status && Object\.prototype\.hasOwnProperty\.call\(todoCond, 'status'\)\) return null/)
