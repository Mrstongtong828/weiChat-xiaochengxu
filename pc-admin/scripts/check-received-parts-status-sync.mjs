...
const recoverySource = readFileSync(resolve(root, 'src/modules/workOrders/receivedPartsStatus.js'), 'utf8')
...
expect(recoverySource.includes("getReceiptStatus(order) === 'confirmed' && receiptSyncSourceStatuses.has(getOrderStatus(order))"), 'missing confirmed pending/sent status guard')
expect(source.includes('const shouldSyncReceivedStatus = (order = {}) => needsReceivedPartsStatusSync(order)'), 'missing receipt status sync predicate')
...
expect(source.includes("const isReceiptStatusRecovery = status === '已签收' && needsReceivedPartsStatusSync(row)"), 'missing historical receipt status recovery')
expect(source.includes('? await syncReceivedOrderStatus(token, row)'), 'historical recovery does not reuse the safe receipt status route')
expect(source.includes("return '同步为已签收'"), 'missing explicit historical recovery action label')
expect(recoverySource.includes("activeTab === 'return' && allowedStatuses.includes('已回寄')"), 'return tab does not prefer return shipment after recovery')
