...
- **Invoice policy**: WeChat Pay orders do not enter the invoice workflow. Only paid/reconciled corporate transfers (`offline_transfer`/`bank_transfer`) may request an invoice, and finance manually issues it outside the system before registering the status, number, date, and file URL with `updateInvoiceStatus`. Automatic/provider invoicing is intentionally unsupported. The shared rule lives in `common/cicada-invoice-policy`.
