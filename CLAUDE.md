...
- `cicada-admin-order` (`adminOrder`): order list/assign/status **plus parts, inventory, settlement, logistics, invoicing, and the four-flow ledger** — `createAdminOrder` supports offline/manual intake and stores the intake date in `ship_out_info.received_at`, while synchronizing a new customer's `biz_user` into CRM (existing non-empty CRM values are preserved); `listParts`/`savePart`/`updatePartStatus`/`listInventoryFlows`/`useOrderParts`/`getSettlementList`; logistics: `batchImportLogistics` (two-segment — `type: 'inbound'` customer-in vs `type: 'return'` ship-back), `getLogisticsLedger`, `getLogisticsExceptions` (48h un-collected / 72h stalled); invoicing: `updateInvoiceStatus` (manual registration for paid corporate transfers only), `getInvoiceApplications`, `batchImportInvoices`; reporting: `getFourFlowLedger` (order+logistics+payment+invoice) and `getDashboardSummary`. Backs `order.js`, `inventory.js`, and `settlement.js`. There is **no** separate inventory/settlement/logistics/invoice cloud function — it all lives here.
...
Core: `cicada_users` (client + staff, `role` field), `cicada_orders`, `cicada_order_items`, `cicada_order_events` (order timeline/audit), `cicada_user_devices`, `cicada_addresses`, `cicada_sn_logs` (SN 扫码/手动查询埋点; `action` `sn_scan`/`sn_query`, `source` `client`/`admin`), `cicada_admin_logs` (admin operation audit: `reset_password`/`staff_*`/`save_settings`; server-write only — schema `permission` is all-`false`, distinct from the order-scoped `cicada_order_events`).
...
- **Invoice policy**: Completed orders with confirmed payment may request an invoice for WeChat Pay (`wechat_pay`) or paid/reconciled corporate transfers (`offline_transfer`/`bank_transfer`). Finance manually issues it outside the system, then registers the status, number, date, and issued-invoice PDF with `updateInvoiceStatus`; the PDF must be uploaded to project cloud storage. The mini program shows structured invoice details and lets the authenticated owner of the issued invoice view/download that PDF. Automatic/provider invoicing is intentionally unsupported. The shared rule lives in `common/cicada-invoice-policy`.
- **External integrations are env-gated**: express/logistics tracking and WeChat Pay (`WX_PAY_*`) depend on console environment variables — treat these as "wire before launch" rather than assuming they work. See the launch-config docs in the repo root.
