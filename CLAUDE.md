...
- `cicada-admin-order` (`adminOrder`): order list/assign/status **plus parts, inventory, settlement, logistics, invoicing, and the four-flow ledger** — `listParts`/`savePart`/`updatePartStatus`/`listInventoryFlows`/`useOrderParts`/`getSettlementList`; logistics: `batchImportLogistics` (two-segment — `type: 'inbound'` customer-in vs `type: 'return'` ship-back), `getLogisticsLedger`, `getLogisticsExceptions` (48h un-collected / 72h stalled); invoicing: `updateInvoiceStatus` (manual registration), `issueInvoice` (calls an external e-invoice provider — see below), `getInvoiceApplications`, `batchImportInvoices`; reporting: `getFourFlowLedger` (order+logistics+payment+invoice) and `getDashboardSummary`. Backs `order.js`, `inventory.js`, and `settlement.js`. There is **no** separate inventory/settlement/logistics/invoice cloud function — it all lives here.
