...
- 财务在 PC 后台“报修工单管理 -> 工单详情 -> 收款”核对银行流水后点击“审核通过”。确认成功后付款状态变为已到账，工单状态允许时自动进入处理中。
- 微信支付由回调或服务端查单确认，禁止后台手工核销，且微信支付订单不提供发票。
- 只有已完成、已核销且金额大于零的对公转账订单可申请发票。财务在线下人工开具，再在工单“发票”页登记结果。
- 税收分类固定为“修理修配劳务”，项目固定为“牙科设备检修服务费”；发票号码不可跨工单重复使用。
- 小程序只展示可查看、可复制的结构化发票摘要，不打开 PDF 外链。电子票原件如需归档，只上传到项目云存储，并由财务通过约定渠道交付客户。

完整后台操作、对公账户和上线验收步骤见 [`uniCloud-alipay/微信支付与一键开票配置.md`](uniCloud-alipay/微信支付与一键开票配置.md)。
...
1. 发票闭环：仅用已完成且已核销的对公转账工单在小程序提交发票申请，确认 `cicada_orders.invoice_info` 写入 `need_invoice=true`、`status=待开票`、抬头、税号、邮箱和备注；微信支付订单不显示申请入口且服务端拒绝登记；后台改为 `已开具` 后，小程序刷新能看到结构化发票信息。
