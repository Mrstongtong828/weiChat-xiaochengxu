...
function getHttpJsonBody(ctx) {
  if (!ctx || typeof ctx.getHttpInfo !== 'function') return {}
  ...
  try {
    ...
    if (httpInfo.isBase64Encoded === true || httpInfo.isBase64Encoded === 'true') {
    ...
    console.warn('解析云对象 HTTP 请求体失败:', e && e.message)
    return {}
  }
}
...
function isDuplicateDocumentError(e) {
  const message = String((e && (e.message || e.errMsg || e.errmsg)) || e || '')
  return /document is already exists|already exists|duplicate|E11000/i.test(message)
}
...
function buildUserInfo(user = {}, id = '') {
  ...
  const nickname = user.nickname || user.name || (phone ? `用户${String(phone).slice(-4)}` : '微信用户')
  return {
    id: userId,
    userId,
    phone,
    ...
    nickname,
    avatar: user.avatar || '',
    avatarFileID: user.avatar_file_id || user.avatarFileID || user.avatarFileId || '',
...
function normalizeUserProfile(input = {}) {
  const source = input.profile || input.userInfo || input
  const nickname = normalizeText(source.nickname || source.nickName || source.name).slice(0, 40)
  const avatar = normalizeText(source.avatar || source.avatarUrl).slice(0, 500)
  const avatarFileID = normalizeText(source.avatarFileID || source.avatarFileId || source.avatar_file_id).slice(0, 500)
  const result = {}
  if (nickname) {
  ...
  if (avatarFileID) result.avatar_file_id = avatarFileID
  return result
}

function getSafeImageExt(fileName = '', fileType = '') {
  const source = `${fileName} ${fileType}`.toLowerCase()
...
function getImageMimeType(ext = 'jpg') {
...
async function ensureCustomerProfile(userId, openid, phone, profile = {}) {
  try {
    ...
    const now = Date.now()
    if (!found.data.length) {
      const customerName = nickname || normalizeText(phone) || (openid ? `微信客户-${String(openid).slice(-6)}` : '微信客户')
      const customerInfo = {
        name: customerName,
        contact: '',
      ...
      try {
        await col.add(customerInfo)
      } catch (e) {
        if (!isDuplicateDocumentError(e)) throw e
        const existed = await col.where(db.command.or(matchOr)).limit(1).get()
        const c = existed.data[0]
        if (c && c._id) {
          await col.doc(c._id).update({
            user_id: userId || c.user_id || '',
            openid: openid || c.openid || '',
            phone: normalizeText(phone) || c.phone || '',
...
async function checkRateLimit(scope, identity, options) {
  ...
  let found = await col.where({ key }).limit(1).get()
  let record = found.data[0]
  ...
  if (!record || now > record.reset_time) {
    if (record) {
      ...
      try {
        ...
        if (!isDuplicateDocumentError(e)) throw e
        found = await col.where({ key }).limit(1).get()
        record = found.data[0]
        if (!record) throw e
...
module.exports = {
  ...
  async login(params = {}) {
    try {
      // 云对象 URL 化时，POST JSON 位于 this.getHttpInfo().body，不会自动进入方法参数。
      // 兼容普通 uniCloud.importObject 调用和 HTTPS URL 调用两种方式。
      const httpParams = getHttpJsonBody(this)
      const payload = {
        ...(params && typeof params === 'object' ? params : {}),
        ...httpParams
      }
      const { code, phoneCode, mockPhone, test } = payload

      // Mock 模式：直接使用传入的 mockPhone，跳过微信接口调用
      if (test && mockPhone) {
        console.log('[Mock模式] 使用测试手机号登录:', mockPhone)
        ...
        await ensureCustomerProfile(userId, mockOpenid, phone)
        ...
        return {
          ...
          data: {
            ...
            userInfo: buildUserInfo({ ...(userRecord || {}), phone, role }, userId)
          }
        }
      }
      ...
      // 2. 换取手机号；未传 phoneCode 时降级为 openid 登录。
      let phone = ''
      try {
        phone = await getPhoneNumberByCode(phoneCode)
      } catch (e) {
        console.log('获取手机号失败，降级为 openid 登录:', e.message)
      }
      ...
      return {
        ...
        data: {
          ...
          userInfo: buildUserInfo({ ...(userRecord || {}), phone: savedPhone, role }, userId)
        }
      }
    } catch (e) {
  ...
  async updateUserInfo(data = {}) {
    try {
      const payload = {
        ...
        ...getHttpJsonBody(this)
      }
      const user = await verifyUserToken(payload.token)
      const profile = normalizeUserProfile(payload)
      if (!Object.keys(profile).length) {
        return { code: -1, msg: '缺少可更新的用户资料' }
      }
      ...
      await ensureCustomerProfile(user._id, user.openid, user.phone, profile)

      return {
        code: 0,
        data: buildUserInfo({ ...user, ...update }, user._id)
      }
    } catch (e) {
      return { code: -1, msg: e.message }
    }
  ...
  async uploadAvatar(data = {}) {
    try {
      ...
      const user = await verifyUserToken(payload.token)
      const rawContent = normalizeText(payload.fileContent || '').replace(/^data:image\/[a-zA-Z0-9.+-]+;base64,/, '')
      if (!rawContent) return { code: -1, msg: '缺少头像文件内容' }
      if (rawContent.length > 8 * 1024 * 1024) return { code: -1, msg: '头像文件过大，请重新选择' }

      const buffer = Buffer.from(rawContent, 'base64')
      if (!buffer.length) return { code: -1, msg: '头像文件读取失败' }

      const ext = getSafeImageExt(payload.fileName, payload.fileType)
      const cloudPath = `user/avatars/${user._id}_${Date.now()}.${ext}`
      const uploadRes = await uniCloud.uploadFile({
        cloudPath,
        fileContent: buffer,
        fileType: getImageMimeType(ext)
      })
      const fileID = uploadRes.fileID || uploadRes.fileId || ''
      if (!fileID) return { code: -1, msg: '头像上传失败' }

      let avatarUrl = fileID
      try {
        const temp = await uniCloud.getTempFileURL({ fileList: [fileID] })
        const item = temp.fileList && temp.fileList[0]
        avatarUrl = (item && item.tempFileURL) || fileID
      } catch (err) {
        avatarUrl = fileID
      }
      ...
      await db.collection('cicada_users').doc(user._id).update(update)
      await ensureCustomerProfile(user._id, user.openid, user.phone, update)
