...
test('completed WeChat Pay orders can prompt the customer to request an invoice', () => {
  const completed = buildSubscriptionData({
    ...order,
    status: 'completed',
  ...
  assert.match(completed.thing4.value, /如需发票，请到工单申请/)
})
...
test('stale invoice state cannot make an ineligible order promise invoice delivery', () => {
  ...
  assert.match(wechat.thing4.value, /发票开具后，请到工单查看/)
