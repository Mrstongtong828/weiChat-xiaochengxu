<template>
	<view class="page-shell">
		<view v-if="activeModule" class="module-page">
			...
			<view v-else-if="activeModule === 'login'" class="module-content login-module login-image-module">
				...
				<button
					...
					:open-type="loginAgreementChecked && !loginSubmitting ? 'getPhoneNumber' : ''"
		...
		<view v-else-if="pageBootReady" class="page-scroll">
			...
			<view v-else-if="activeTab === 'company'" class="company-body">
				...
				<view class="company-hero">
					<image class="company-hero-image" :src="cicadaAssets.companyIntroHeader" mode="aspectFill"></image>
				...
				<view class="mine-hero">
					<view class="profile-row">
						<view class="avatar" :class="{ 'avatar-logged': logged }">
							<image v-if="logged && userAvatarUrl" class="avatar-image" :src="userAvatarUrl" mode="aspectFill"></image>
							<text v-else-if="logged">{{ userAvatarText }}</text>
							<image v-else class="avatar-image" :src="cicadaAssets.defaultUserAvatar" mode="aspectFit"></image>
						</view>
						<view class="profile-copy">
							...
							<view v-if="logged" class="profile-meta">
								...
								<view class="member-tag profile-edit-tag tap" @click="editWechatProfile">修改资料</view>
								<view class="logout-btn tap" @tap.stop="logoutLocal">退出</view>
							</view>
		...
		/>
		<WechatProfileInlineEditor v-if="false" />
		<PrivacyConsent />
...
import WechatProfileInlineEditor from '@/components/WechatProfileInlineEditor.vue'
...
const userAvatarUrl = computed(() => currentUser.value.avatar || currentUser.value.avatarUrl || currentUser.value.avatarFileID || currentUser.value.avatar_file_id || '')
...
const onGetPhoneNumberLogin = async (event = {}) => {
	...
	try {
		...
		if (await applyLoginSession(res)) {
			loginError.value = ''
...
const resolveUserAvatarFromCloud = async (userInfo = {}) => {
	const avatarFileID = userInfo.avatarFileID || userInfo.avatarFileId || userInfo.avatar_file_id || (isCloudFileId(userInfo.avatar) ? userInfo.avatar : '')
	if (!avatarFileID) return

	try {
		const avatarUrl = await resolveGuideFileUrl(avatarFileID)
		if (!avatarUrl || avatarUrl === currentUser.value.avatar) return
		currentUser.value = {
			...currentUser.value,
			avatar: avatarUrl,
			avatarUrl,
		...
		console.warn('resolve user avatar failed:', error)
	}
}
...
const syncAuthStateFromStorage = () => {
	...
	currentUser.value = token ? userInfo : {}
	if (token) resolveUserAvatarFromCloud(userInfo)
}

const onAuthChanged = (payload = {}) => {
	if (payload.logged === false) {
		logoutLocal({ silent: true })
		return
	}
...
const onProfileEditorSaved = (userInfo) => {
	...
	currentUser.value = userInfo || currentUser.value
	uni.setStorageSync('userInfo', currentUser.value)
	uni.$emit('auth:changed', { logged: true, userInfo: currentUser.value })
}
...
const editWechatProfile = () => {
	uni.navigateTo({
		url: '/pages-sub/profile/index',
		fail: () => uni.showToast({ title: '修改资料页面暂不可用', icon: 'none' })
	})
}

const applyLoginSession = async (res = {}, options = {}) => {
	if (!res || !res.token) {
...
const loginWithWechatIdentity = async () => {
	...
	try {
		...
		console.warn('wechat identity login failed:', error)
...
const onDevLogin = async () => {
	try {
		...
		if (await applyLoginSession(res, { syncWechatProfile: false })) {
...
const logoutLocal = (options = {}) => {
	...
	logged.value = false
	if (!options.silent) {
		uni.$emit('auth:changed', { logged: false })
	}
}
...
onLoad((options = {}) => {
	...
	const targetTab = String(options.tab || '')

	if (targetTab && tabRoutes[targetTab]) {
		activeTab.value = targetTab
		activeModule.value = ''
		previousModule.value = ''
	}

	if (options.editProfile === '1' || options.editProfile === 'true') {
		setTimeout(() => {
...
onMounted(() => {
	...
	uni.$on('auth:changed', onAuthChanged)
	syncAuthStateFromStorage()
...
onUnmounted(() => {
	...
	uni.$off('auth:changed', onAuthChanged)
})
</script>
