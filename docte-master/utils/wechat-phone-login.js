...
const EXPIRED_CODE_PATTERNS = [
	/40029|40163|invalid code|code been used|code is expired|js_code/i,
	/授权凭证|登录凭证|凭证已失效|code/i
]
...
const getRawErrorMessage = (error) => String((error && (error.message || error.errMsg || error.msg)) || '登录失败')

export const isExpiredLoginCodeError = (error) => {
	const message = getRawErrorMessage(error)
	return EXPIRED_CODE_PATTERNS.some((pattern) => pattern.test(message))
}

export const getLoginErrorMessage = (error) => {
	const message = getRawErrorMessage(error)
	if (/WX_APPID|WX_SECRET|AppID|Secret/i.test(message)) {
		return '登录服务未配置微信小程序 AppID/Secret，请检查 uniCloud 环境变量'
	}
	if (/手机号能力|手机号授权|phoneCode|getuserphonenumber|phone number/i.test(message)) {
		return message
	}
	if (isExpiredLoginCodeError(error)) {
		return '授权凭证已失效，请重新点击微信手机号授权登录'
	}
	if (/access_token|40001|42001/i.test(message)) {
		return '微信接口凭证失效，请稍后重试；若持续失败请检查小程序后台配置'
	}
	return message
}
...
export const loginWithWechatPhoneCode = async (wechatLogin, phoneCode, options = {}) => {
	...
	let lastError = null
	let refreshedCodeAfterExpired = false
	let fallbackToOpenidTried = false
	...
	for (let attempt = 0; attempt <= retries; attempt += 1) {
		try {
			...
			if (!refreshedCodeAfterExpired && isExpiredLoginCodeError(error)) {
				refreshedCodeAfterExpired = true
			...
			if (!fallbackToOpenidTried && isExpiredLoginCodeError(error)) {
				fallbackToOpenidTried = true
				if (typeof options.onRetry === 'function') options.onRetry(error, attempt + 1)
				try {
					const code = await requestWechatLoginCode()
					return await wechatLogin({ code, phoneCode: '' })
...
export const loginWithWechatOpenid = async (wechatLogin, options = {}) => {
	...
	for (let attempt = 0; attempt <= retries; attempt += 1) {
		try {
			...
			lastError = error
			if (attempt < retries && (isRetryableLoginError(error) || isExpiredLoginCodeError(error))) {
				if (typeof options.onRetry === 'function') options.onRetry(error, attempt + 1)
				await wait(600)
				continue
			}
