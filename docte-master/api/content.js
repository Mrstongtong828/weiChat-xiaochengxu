...
import { callCloudObjectHttp, getCloudTempFileURL, importCloudObject, checkCloudAvailable } from '@/utils/cloud.js'
...
const isCloudUnavailableError = (error) => /云服务暂不可用|云服务未连接|云服务未初始化|uniCloud 服务空间|importObject|uniCloud/i.test(String(error && (error.message || error.errMsg || error)))

const loginByHttp = (data = {}) => callCloudObjectHttp('cicada-client-user', 'login', data)
	.then(persistLoginSession)

export const wechatLogin = (data = {}) => {
	// 微信小程序真机/预览优先走 URL 化云对象，避免依赖 HBuilderX 本地 uniCloud 关联状态。
	// #ifdef MP-WEIXIN
	return loginByHttp(data)
	// #endif
	...
	try {
		...
		if (allowOfflineLoginFallback() && isCloudUnavailableError(error)) {
			console.warn('cloud login unavailable, using offline session:', error)
			return Promise.resolve(persistLoginSession(getLocalDevLoginSession()))
		}
...
export const devLogin = async () => {
	try {
		...
		if (!cloudObject || typeof cloudObject.devLogin !== 'function') {
			return persistLoginSession(getLocalDevLoginSession())
		}
		return await cloudObject.devLogin({}).then(unwrapCloudResult).then(persistLoginSession)
	} catch (error) {
		if (isCloudUnavailableError(error)) {
...
const updateUserInfoByHttp = (data = {}) => callCloudObjectHttp('cicada-client-user', 'updateUserInfo', data)
	.then(persistUserInfo)

const getFileNameFromPath = (filePath = '') => {
	const clean = String(filePath || '').split('?')[0]
	const name = clean.split(/[\\/]/).filter(Boolean).pop() || ''
	return /\.[a-zA-Z0-9]+$/.test(name) ? name : `avatar_${Date.now()}.jpg`
}
...
export const uploadUserAvatar = async (filePath = '') => {
	...
	if (!fileContent) throw new Error('头像文件读取失败')
	return callCloudObjectHttp('cicada-client-user', 'uploadAvatar', {
		fileContent,
		fileName,
...
export const updateUserInfo = (data = {}) => {
	// #ifdef MP-WEIXIN
	return updateUserInfoByHttp(data)
	// #endif

	try {
		const cloudObject = getUserCloudObject()
		if (!cloudObject || typeof cloudObject.updateUserInfo !== 'function') {
			return updateUserInfoByHttp(data)
		}
		return cloudObject.updateUserInfo(withToken(data))
