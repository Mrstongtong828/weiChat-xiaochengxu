		...
		<view class="mine-hero">
			...
			<view class="profile-row">
				<view class="avatar" :class="{ 'avatar-logged': logged }">
					<image v-if="logged && userAvatarUrl" class="avatar-empty" :src="userAvatarUrl" mode="aspectFill"></image>
					<text v-else-if="logged">{{ userAvatarText }}</text>
					<image v-else class="avatar-empty" :src="cicadaAssets.defaultUserAvatar" mode="aspectFit"></image>
				</view>
...
const resolveUserAvatarFromCloud = async (userInfo = {}) => {
	...
	try {
		const avatarUrl = await resolveCloudFileUrl(avatarFileID)
...
const syncLoginState = () => {
	...
	if (token) resolveUserAvatarFromCloud(currentUser.value)
	if (token) {
		loadRepairCounts()
		loadProductCount()
	}
...
const onAuthChanged = () => syncLoginState()
...
const menus = computed(() => [
	{ icon: 'edit', title: '修改资料', desc: '设置微信头像与昵称', go: 'profile' },
...
const routes = {
	...
	profile: '/pages-sub/profile/index',
